...
class CreateTableLifePrivilege20170721 extends Migration
{
    ...
    public function up()
    {
        Schema::create('life_privilege', function (Blueprint $table) {
            ...
            $table->tinyInteger('debit_status',false,true)->default(0)->comment("网利宝扣款状态0未扣款 1已扣款 2已退款");
            $table->tinyInteger('order_status',false,true)->default(0)->comment("订单状态0未充值 1正在充值 2充值失败 3充值成功 4异常状态");
            $table->tinyInteger('repair_status',false,true)->default(0)->comment("订单状态0未补单 1已补单");
            $table->integer('repair_id',false,true)->nullable()->default(NULL)->comment("补单id");
            $table->text('remark')->default(NULL)->comment("用户扣钱备注");
            $table->text('remark_of')->default(NULL)->comment("殴飞接口备注");
            $table->string('order_time')->default(NULL)->comment('订单回调时间');
            $table->string('order_remark')->default(NULL)->comment('订单回调消息');
            $table->timestamps();
            //索引
            $table->index('user_id');
            $table->index('phone');
