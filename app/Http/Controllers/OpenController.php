...
class OpenController extends Controller
{
    ...
    public function postAyqRegister(Request $request){
        ...
        $verifRes = $client->verified(array('name'=>json_decode($realname),'id_number'=>$cardno));
        if(isset($verifRes['error']) && in_array($verifRes['error']['code'],array(1106,1112,1206,1209,1210,1405))){
            file_put_contents(storage_path('logs/verifed-'.date('Y-m-d')).'.log',date('Y-m-d').'  code:'.$verifRes['error']['code'].'  msg:'.$verifRes['error']['message'].'  name:'.$realname.PHP_EOL,FILE_APPEND);
