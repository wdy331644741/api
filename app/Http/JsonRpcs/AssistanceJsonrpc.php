...
class AssistanceJsonRpc extends JsonRpc
{
    ...
    public function assistanceReceive($params){
        ...
        if(isset($groupInfo['id'])){//已满团
            $userStatus = HdAssistance::where("pid",$groupInfo['id'])->where("user_id",$userId)->where('status',1)->first();
            if(isset($userStatus['id'])){//领取成功
                //修改领取状态
                HdAssistance::where("pid",$groupInfo['id'])->where("user_id",$userId)->update(['receive_status'=>1,'updated_at'=>date("Y-m-d H:i:s")]);
                return array(
                    'code' => 0,
                    'message' => 'success',
                    'data' => true
