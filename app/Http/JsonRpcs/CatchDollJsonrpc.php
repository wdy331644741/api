...
class CatchDollJsonRpc extends JsonRpc
{
     ...
     */
    protected $old_award = [
        ['alias_name' =>'catch_doll_8888_ex', 'desp' => '8888元体验金', 'size' => 8888, 'pro' => 100],
        ['alias_name' =>'catch_doll_5_ca', 'desp' => '5元现金', 'size' => 5, 'pro' => 0],
    ];
    /**
     *  新用户中奖概率
     */
    ...
    public function gameDollInfo() {
        ...
        if($res['is_login'] == true){
            ...
            //今天是否已经分享过
            $res['is_share_game'] = isset($attr->text) && ($attr->text > date('Y-m-d') ) ?true:false;
        }
