...
class OpenGiftJsonRpc extends JsonRpc
{
    ...
    protected $award_list = [
        ['alias_name' =>'cash', 'desp' => '688.88现金', 'size' => 688.88, 'pro' => 1],
        ['alias_name' =>'cash', 'desp' => '68.88现金', 'size' => 68.88, 'pro' => 10],
        ['alias_name' =>'cash', 'desp' => '1.88现金', 'size' => 1.88, 'pro' => 477],
        ['alias_name' =>'gift', 'desp' => '100京东卡', 'size' => 100, 'pro' => 7],
        ['alias_name' =>'gift', 'desp' => '50京东卡', 'size' => 50, 'pro' => 15],
        ['alias_name' =>'gift', 'desp' => '爱奇艺会员卡', 'size' => 0, 'pro' => 40],
        ['alias_name' =>'increases', 'desp' => '1%加息券', 'size' => 0.01, 'pro' => 150],
        ['alias_name' =>'increases', 'desp' => '2%加息券', 'size' => 0.02, 'pro' => 130],
        ['alias_name' =>'redMoney', 'desp' => '108元红包', 'size' => 108, 'pro' => 170],
    ];
    ...
    public function openGiftInfo() {
        ...
        if($userId > 0){
            ...
            $_award = SendRewardLog::select('remark','created_at')->where('user_id',$userId)->where('status','>=',1)->where('activity_id',$_act->id)->orderBy('created_at','DESC')->get()->toArray();
            $res['list'] = $this->transformJson($_award);
        }
    ...
    public function openGiftDraw() {
        ...
        $award = $this->getAward($this->award_list);
    ...
    private function allUserList() {
        ...
        return Cache::remember($key,10, function() {
            $_act = ActivityService::GetActivityInfoByAlias(self::$attr_key);//获取活动id
            $tmp = SendRewardLog::select('user_id','remark')->where('status','>=',1)->where('activity_id',$_act->id)->orderBy('created_at','DESC')->get()->toArray();
            $newArr = [];
            if(!empty($tmp)){
                foreach ($tmp as $key => $value) {
                    $phone = protectPhone(Func::getUserPhone($value['user_id']) );
                    $jsonData = json_decode($value['remark'],1);
                    $newArr[$key]['user'] = $phone;
                    $newArr[$key]['award'] = $jsonData['award_name'];
                }
